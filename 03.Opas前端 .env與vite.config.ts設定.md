...
## 4. 各專案Component常用變數與函數

前端可以透過`import.meta.env.VITE_PROJECT`與`import.meta.env.VITE_MVC_PORT`取得.env中的設定值，如下：

```ts
import { requestURL } from "./utils/requestURL";
const port = import.meta.env.VITE_MVC_PORT as string;
const projectName = import.meta.env.VITE_PROJECT as string;

const url = requestURL(projectName, prot)    // 透過requestURL組合成後端API的URL
```

以下是requestURL的內容：

```ts
const requestURL = (controller: string) => {
  const baseUrl =
    import.meta.env.MODE == "development"
      ? "http://localhost:" + import.meta.env.VITE_MVC_PORT
      : location.origin;
  const projectName = location.origin.includes("localhost")
    ? ""
    : "/" + import.meta.env.VITE_PROJECT;
  return baseUrl + projectName + controller;
};
```
